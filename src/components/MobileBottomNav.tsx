...
export const MobileBottomNav: React.FC = () => {
  ...
  // If unauthenticated, do NOT expose mobile bottom app navigation
  ...
  const homeDashboard = getRoleDashboardPath(user?.role || role);
  ...
  return (
    <aside 
      ...
      className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-[#0C0C0E]/95 backdrop-blur-xl border-t border-ner-border/90 dark:border-zinc-800 px-2 pt-1.5 pb-[max(0.5rem,env(safe-area-inset-bottom))] shadow-[0_-4px_20px_rgba(0,0,0,0.06)] dark:shadow-[0_-4px_20px_rgba(0,0,0,0.5)]"
    >
          ...
          return (
            <NavLink
              ...
              className={`flex flex-col items-center justify-center py-1 px-3 rounded-2xl min-w-[56px] min-h-[48px] transition-all duration-200 active:scale-95 ${
                isActive 
                  ? 'text-ner-terracotta font-bold' 
                  : 'text-ner-black/60 dark:text-zinc-400 hover:text-ner-black dark:hover:text-white'
              }`}
            >
              <div className={`p-1.5 rounded-xl transition-colors ${isActive ? 'bg-ner-terracotta/15 text-ner-terracotta' : ''}`}>
